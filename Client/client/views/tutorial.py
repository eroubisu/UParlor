...
_KEYS_HORZ_NEXT = f'{M_DIM}下一标签  {M_CMD}l{M_END}{M_DIM}    {M_CMD}→{M_END}{M_DIM}    {M_CMD}d{M_END}'
_KEYS_HORZ_PREV = f'{M_DIM}上一标签  {M_CMD}h{M_END}{M_DIM}    {M_CMD}←{M_END}{M_DIM}    {M_CMD}a{M_END}'
...
# ── Pages ──
_TITLES = ['欢迎', '列表面板', '文本面板', '标签页', '输入模式', '快捷菜单', '完成']
_TOTAL = len(_TITLES)
_P_SCROLL = 2
_P_TAB = 3
_P_INPUT = 4
...
def _page_welcome() -> list[tuple[str, str]]:
    return [
        (TEXT, _KEYS_VERT),
        (TEXT, f'{M_DIM}确认选择  {M_CMD}Enter{M_END}{M_DIM} / {M_CMD}z{M_END}'),
        (SEP, ''),
        (TEXT, f'{M_BOLD}选择操作方式{M_END}'),
...
def _page_list(p: int) -> list[tuple[str, str]]:
    return [
        (TEXT, f'按 {M_CMD}Enter{M_END} 选中 / 取消'),
        (SEP, ''),
        (TEXT, f'{M_BOLD}选中全部选项{M_END}'),
        (PRACTICE, '选项 A'),
        (PRACTICE, '选项 B'),
        (PRACTICE, '选项 C'),
        (PRACTICE, '选项 D'),
...
def _page_input(p: int, messages: list[str]) -> list[tuple[str, str]]:
    items: list[tuple[str, str]] = [
        (TEXT, f'{M_CMD}i{M_END} 输入，{M_CMD}Enter{M_END} 发送'),
        (TEXT, f'{M_DIM}大写 {M_CMD}I{M_END}{M_DIM} = 持续输入{M_END}'),
        (SEP, ''),
...
def _page_space(p: int) -> list[tuple[str, str]]:
    return [
        (TEXT, f'{M_CMD}Space{M_END} 快捷菜单'),
        (TEXT, f'{M_CMD}Space → w{M_END} 切换面板焦点'),
        (SEP, ''),
        (BACK, _LABEL_BACK),
        (ACTION, _LABEL_NEXT),
    ]
...
def _page_done(p: int) -> list[tuple[str, str]]:
    return [
        (TEXT, f'{M_BOLD}教程完成！{M_END}'),
        (SEP, ''),
        (TEXT, f'{M_CMD}Esc{M_END} / {M_CMD}q{M_END} 关闭窗口'),
        (TEXT, f'{M_DIM}详见 {M_CMD}Space → s → 打开文档{M_END}'),
        (SEP, ''),
        (BACK, _LABEL_BACK),
    ]


_BUILDERS = [_page_welcome, _page_list, None, _page_tab, None, _page_space, _page_done]
...
class TutorialPanel(Panel):
        ...
        if self._page == _P_TAB:
            ...
            if action == 'tab_prev':
                if self._on_tab1:
                    ...
                    self._cursor = -1  # no selectable on tab0
                    self.call_after_refresh(lambda: self._redraw(tab=0))
                else:
                    self._goto_page(_P_SCROLL)  # back to scroll page
                return
    ...
    def _activate(self) -> None:
        ...
        match self._page:
            case 0:  # welcome
                if not self._pref_set:
                    return
                self._goto_page(1)
            case 1:  # list practice
                prac = [i for i, (t, _) in enumerate(self._items) if t == PRACTICE]
                if not all(i in self._checked for i in prac):
                    return
                self._goto_page(_P_SCROLL)
            case 3:  # tab (action is on tab1)
                self._goto_page(_P_INPUT)
            case 4:  # input — need ≥1 message
    ...
    def _build_page(self) -> None:
        ...
        if builder is None:
            return
        self._items = builder() if self._page == 0 else builder(self._pref)
        self._cursor = self._first_selectable()
        self._redraw()
