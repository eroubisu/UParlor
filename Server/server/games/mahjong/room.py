...
class MahjongRoom:
    ...
    def get_round_name(self) -> str:
        ...
        suffix = f' {self.honba}本场' if self.honba > 0 else ''
    ...
    def check_tsumo(self, seat: int) -> dict | None:
        ...
        try:
            ...
            result = _calculator.estimate_hand_value(
                full, self.drawn_tile, melds=melds, config=config)
            if result.error:
                return None
            return {
                'han': result.han,
    ...
    def check_ron(self, seat: int, tile_136: int) -> dict | None:
        ...
        try:
            ...
            melds = self._build_melds_for_calc(seat)
            result = _calculator.estimate_hand_value(
                full, tile_136, melds=melds, config=config)
            if result.error:
                return None
            return {
                'han': result.han,
    ...
    def get_game_data(self, seat: int) -> dict:
        ...
        data = {
            ...
            'my_turn': self.current_turn == seat,
            'round_name': self.get_round_name(),
