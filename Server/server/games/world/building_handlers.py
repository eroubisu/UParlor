...
def _load_shops() -> dict:
    path = os.path.join(_dir, 'shops.json')
...
def _menu_event(title, items, empty_msg=''):
    """构建 select_menu game_event 返回值"""
    return {
        'action': 'select_menu',
        'send_to_caller': [{
            'type': 'game_event',
...
def cmd_buy(lobby, player_name, player_data, args, location):
    ...
    if not args:
        ...
        for entry in buy_list:
            ...
            items.append({'label': f"{name} — {entry['price']}G", 'command': f"/buy {entry['id']}"})
        return _menu_event('可购买的商品', items, '商店里空空如也。')

    item_id = args.strip().split()[0]
...
def cmd_forge(lobby, player_name, player_data, args, location):
    ...
    if not args:
        ...
        for recipe_id, recipe in _get_recipes().items():
            ...
            label = f"{recipe['name']} — {inputs_desc} + {gold}G"
            items.append({'label': label, 'command': f'/forge {recipe_id}'})
        return _menu_event('锻造装备', items, '没有可用的配方。')

    recipe_id = args.strip()
...
def cmd_brew(lobby, player_name, player_data, args, location):
    if location != 'world_herbshop':
        return "只有在药草店才能炼药。"
    if not args:
        items = [
            {'label': '治疗草药x2 → 经验药水x1', 'command': '/brew exp_potion'},
        ]
        return _menu_event('炼药', items)

    inventory = player_data.get('inventory', {})
    if args.strip() == 'exp_potion':
