...
class DoudizhuEngine(BaseGameEngine):
    ...
    def _play_select_menu(self, room, player_name):
        """构建出牌选择菜单"""
        from .patterns import PASS, find_all_beats, doudizhu_rank
        hand = room.hands[player_name].cards
        items: list[dict] = []

        need_beat = room.last_play and room.last_play.type_id != PASS
        if need_beat:
            # 需要压过上家：展示推荐出牌组合
            suggestions = find_all_beats(hand, room.last_play)
            for sug in suggestions:
                if sug.get('type') == 'pass':
                    items.append({'label': '不出', 'command': '/pass'})
                elif sug.get('type') == 'single_hint':
                    # 单张可选太多，逐张列出
                    for i, c in enumerate(hand):
                        if doudizhu_rank(c) > room.last_play.main_rank:
                            items.append({
                                'label': c.name,
                                'command': f'/play {i}',
                            })
                else:
                    cards_str = ' '.join(sug['cards'])
                    idx_str = ' '.join(str(i) for i in sug['indices'])
                    items.append({
                        'label': cards_str,
                        'desc': sug['type'],
                        'command': f'/play {idx_str}',
                    })
        else:
            # 自由出牌：列出每张手牌
            for i, c in enumerate(hand):
        ...
        return self._select_menu('出牌', items)
